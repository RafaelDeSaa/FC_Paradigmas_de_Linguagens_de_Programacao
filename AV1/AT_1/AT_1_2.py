def valor_calibracao(linha):
   ...
   # aqui verifica cada caractere de cada linha
    for vnum in linha:
        if vnum.isdigit():
            if primeiro_digito is None:
                primeiro_digito = vnum
            ultimo_digito = vnum

    # Combina o primeiro e último dígito para formar o número de calibração
    if primeiro_digito and ultimo_digito:
        return int(primeiro_digito + ultimo_digito)
    else:
        return 0
...
# aqui Calcula a soma dos valores de calibração
soma_calibracao = sum(valor_calibracao(linha) for linha in listas)

print("Soma da calibração:", soma_calibracao)